...
- (IBAction)opslaan:(id)sender {
    ...
    @try {
        ...
        if([[_tekstveld text] isEqualToString:@""]) {
            [self alertStatus:@"Please fill in all field" :@"Register Failed!"];
        } else {
            NSString *post =[[NSString alloc] initWithFormat:@"item=%@&date=%@",[_tekstveld text], date_String];
            NSLog(@"PostData: %@",post);
            
            NSURL *url=[NSURL URLWithString:@"http://gandibleux.eu/PHP/VoegDagboekToe.php"];
...
- (IBAction)backgroundClicker:(id)sender {
    [_tekstveld resignFirstResponder];
}
